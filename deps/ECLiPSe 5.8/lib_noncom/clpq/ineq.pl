...
ineq_one_new( strict, X, K, Bound) :-
  arith_eval( 1, One),
  ( arith_eval( K < 0) ->
      var_intern( t_l(Bound), X, _, 2'10)
  ;
      var_intern( t_u(Bound), X, _, 2'01)
  ).
ineq_one_new( nonstrict, X, K, Bound) :-
  arith_eval( 1, One),
  ( arith_eval( K < 0) ->
      var_intern( t_l(Bound), X, _, 2'00)
  ;
      var_intern( t_u(Bound), X, _, 2'00)
  ).
...
ineq_one_s_p_0( X) :-
  solver_variable( X, LinX, Xs),
  !,						% old variable, this is deref
  decompose( LinX, OrdX, _, Ix),
  ineq_one_old_s_p_0( OrdX, Xs, Ix).
ineq_one_s_p_0( X) :-				% new variable, nothing depends on it
  arith_eval( 0, Zero),
  var_intern( t_u(Zero), X, _, 2'01).

ineq_one_s_n_0( X) :-
  solver_variable( X, LinX, Xs),
  !,
  decompose( LinX, OrdX, _, Ix),
  ineq_one_old_s_n_0( OrdX, Xs, Ix).
ineq_one_s_n_0( X) :-
  arith_eval( 0, Zero),
  var_intern( t_l(Zero), X, _, 2'10).

ineq_one_s_p_i( X, I) :-
  solver_variable( X, LinX, Xs),
  !,
  decompose( LinX, OrdX, _, Ix),
  ineq_one_old_s_p_i( OrdX, I, Xs, Ix).
ineq_one_s_p_i( X, I) :-
  arith_eval( -I, Bound),
  var_intern( t_u(Bound), X, _, 2'01).
...
ineq_one_s_n_i( X, I) :-
  ...
  !,
  decompose( LinX, OrdX, _, Ix),
  ineq_one_old_s_n_i( OrdX, I, Xs, Ix).
ineq_one_s_n_i( X, I) :-
  var_intern( t_l(I), X, _, 2'10).
...
ineq_one_n_p_0( X) :-
  ...
  !,						% old variable, this is deref
  decompose( LinX, OrdX, _, Ix),
  ineq_one_old_n_p_0( OrdX, Xs, Ix).
ineq_one_n_p_0( X) :-				% new variable, nothing depends on it
  arith_eval( 0, Zero),
  var_intern( t_u(Zero), X, _, 2'00).

ineq_one_n_n_0( X) :-
  solver_variable( X, LinX, Xs),
  !,
  decompose( LinX, OrdX, _, Ix),
  ineq_one_old_n_n_0( OrdX, Xs, Ix).
ineq_one_n_n_0( X) :-
  arith_eval( 0, Zero),
  var_intern( t_l(Zero), X, _, 2'00).

ineq_one_n_p_i( X, I) :-
  solver_variable( X, LinX, Xs),
  !,
  decompose( LinX, OrdX, _, Ix),
  ineq_one_old_n_p_i( OrdX, I, Xs, Ix).
ineq_one_n_p_i( X, I) :-
  arith_eval( -I, Bound),
  var_intern( t_u(Bound), X, _, 2'00).
...
ineq_one_n_n_i( X, I) :-
  ...
  !,
  decompose( LinX, OrdX, _, Ix),
  ineq_one_old_n_n_i( OrdX, I, Xs, Ix).
ineq_one_n_n_i( X, I) :-
  var_intern( t_l(I), X, _, 2'00).
...
%
% Xs are solver internal vars already
%
...
ineq_more( strict, Lind) :-
   ( unconstrained( Lind, U,K, Rest) -> 	% never fails, no implied value
       arith_eval( 0, Z),
       arith_eval( 1, One),
       var_intern( t_l(Z), S, Ss, 2'10),
       arith_eval( -1/K, Ki),
       add_linear_ff( Rest, Ki, [Z,Z,Ss*One], Ki, LinU),
