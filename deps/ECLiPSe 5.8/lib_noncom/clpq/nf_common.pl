...
{ L  =	R } :- !, nf( L-R, Nf), submit_eq( Nf).
{   Rel   } :- raise_exception( undefined_constraint({Rel})).
...
negate( A=B,   A=\=B) :- !.
negate( A=\=B, A=:=B) :- !.
negate( Rel,   _) :- raise_exception( undefined_constraint({Rel})).

%
% chip, lib(r) compatibility
%
L $<  R :- nf( L-R, Nf), submit_lt( Nf).
L $>  R :- nf( R-L, Nf), submit_lt( Nf).
L $=< R :- nf( L-R, Nf), submit_le( Nf).
...
nl_invertible( exp(B,C), X, A, Res) :-
  ...
  ; nf_constant( C, Kc),
      (arith_eval(A=:=0),arith_eval(Kc=<0) -> fail ; true ), % goal expansion
      X = B,
...
%
nf( Term, _) :- raise_exception( undefined_interpreted_term(Term)).
...
%
nf_split( [],	     [], Z) :- arith_eval( 0, Z).
nf_split( [First|T], H,  I) :-
  ( First=v(I,[]) ->
      H=T
  ;
      arith_eval( 0, I),
