...
% Version:	$Id: quintus.pl,v 1.29 2004/11/19 14:27:29 js10 Exp $
% ----------------------------------------------------------------------
...
:- reexport cio.
:- reexport foreign.
:- sepia_kernel:use_module(library(scattered)).	% handler settings only
...
external_key(Key, KeyN) :-
	...
	char_int(NS, 0),		% can't write "\000" in this syntax
...
prolog_flag_body(fileerrors, Old, _) :-
	(get_error_handler(170, nofileerrors_handler/2, _) ->
		Old = off
	;
		Old = on
	).
