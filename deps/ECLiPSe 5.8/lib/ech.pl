...
% Version:	$Id: ech.pl,v 1.18 2002/11/20 18:40:04 ks15 Exp $
% ----------------------------------------------------------------------
...
:- op(1100, fy, handler).
:- op(1000, fy, constraints).
%:- op(1200, fy, [label_with]).
:- op(1190, xfx, [==>, <=>]).
%:- op(1200, xfx, :--).
:- op(1200, xfx, ::=). % replaces @ in old syntax
:- op(1100, xfx, |).
:- op(1100, xfx, \ ).
:- op(700, xfx, flag).

:- local struct(chrcinfo(spec,count,prio)).
:- local variable(chr_priority).
:- setval(chr_priority,9).
...
:- define_macro((handler)/2, ignore_handler/3, [clause]).
:- define_macro((==>)/2, record_chrprule/3, [clause]).
:- define_macro((<=>)/2, record_chrrule/3, [clause]).
:- define_macro((::=)/2, record_namedrule/3, [clause]).
:- define_macro((constraints)/1, new_constraints/3, [clause]).
:- define_macro(no_macro_expansion(constraint_in_wrapper_pos/0), realconstraintpos/2, []).
:- define_macro(no_macro_expansion(suspendid_pos/0), suspendindexpos/2, []).
:- define_macro(no_macro_expansion(constraintnum_pos/0), constraintnumpos/2, []).
:- define_macro(no_macro_expansion(applied_pos/0), appliedpos/2, []).
:- define_macro(no_macro_expansion(wrapper_inpos/0), wrapperinpos/2, []).
%:- define_macro(varslist_threshold/0, varslistthreshold/2, []).
:- define_macro(no_macro_expansion(cdelete_threshold/0), cdeletethreshold/2, []).
...
:- current_array(constraint_number, _) -> true ; make_array(constraint_number), setval(constraint_number, 0).
...
chrcompile_by_module([Module|Modules]) :-
   ...
   (current_array('CHRcdelete_count'(OldSize),_)@Module ->
      (OldSize \== ArraySize ->
         ...
         make_local_array('CHRcdelete_count'(ArraySize), integer)@Module
       ; true
      )
     ;make_local_array('CHRcdelete_count'(ArraySize), integer)@Module
...
translate_each_constraint(N, Processed, Constraints, Nprule, Module) :-
   ...
   optimise(Code0, Code), 
   %printcode(Code), 
   call(compile_term([(:- pragma(expand))|Code]), Module),
   N1 is N - 1, 
...
add_check_in_store(Constraint, Call, No, Prio, Check, Module) :-
   ...
   (recorded('CHRdont_in_store', Module) ->
         Check = PriCall
       ; Check = (call_explicit(get_global_constraint(No, SuspL), ech), 
                   (call_explicit(is_in_store(SuspL, Constraint), ech) -> true ; PriCall)
                 )
   ).
...
gen_code_for_constraint([], PreviousTryNexts, F, A, Nth, NRule, Nprule, CodeTail, Module) :- !,
  % need to generate termination rule
    ...
    CodeTail = [( NewHead :-
      (var(Index) -> 
          call_explicit(suspend_constraint(Constraint, Head, ConstNo, Prio),ech)
        ; suspension_to_goal(Index, Goal, _),
...
% partners
   ...
   (Common \== []  ->
        GetSList = call_explicit(get_constraint_list(Common, ConstNo, SuspL), ech)
   ; 
        GetSList = call_explicit(get_global_constraint(ConstNo, SuspL), ech)
   ),
   construct_check_identical(Partner, Head, PartnerIndex, CIndex, CheckIdentical),
   construct_not_already_matched(MatchedPIdxs, PartnerIndex, CheckNotAlreadyMatched),
   Code = 
      (GetSList,
       call_explicit(find_partner(SuspL, PartnerIndex, Candidate, Rest), ech),
       InstanceCheck,
       CheckIdentical,
...
construct_other_findpartners([Rest|Rests], [Partner|Partners], a(Head,CHead, 
   ...
   construct_not_already_matched(MatchedPIdxs, PartnerIndex, CheckNotAlreadyMatched),
   Code = ( 
      call_explicit(find_partner(Rest, PartnerIndex, Candidate, NewRest), ech),
      CheckIns,
      CheckIdentical,
      CheckNotAlreadyMatched,
...
construct_applied_check(1, double(H1,_H2), Head, RuleNo, PList, [PIndex], Index, Add_Applied, Not_AppliedCheck, AppTail, GConsNo, Status) :- !,
	...
	Not_AppliedCheck = (
	    suspension_to_goal(PIndex, PGoal,_),
	    ...
	    (var(Index) -> /* if var, then initiating head is not in constraint store */
	        ...
	        call_explicit(check_samepairapplied_direct(PList, PListRest, Pos-PConsNumber, AppTail, AppRest, Status), ech)

	      ; /* can check and mark if initiating head already in constraint store */
	        suspension_to_goal(Index, Cons0, _),
                arg(constraintnum_pos, Cons0, GConsNo),
              
	        call_explicit(check_samepairapplied_directmarked(PList, PListRest, Pos-PConsNumber, AppTail, OtherPos-GConsNo, PGoal, RuleNo, Status), ech)
            )
...
construct_applied_check(1, _, _, RuleNo, PList, [PIndex], Index, Add_Applied, 
	...
	Code = (suspension_to_goal(PIndex, PGoal,_),
          arg(constraintnum_pos, PGoal, PConsNumber),
          PList = [_|PListRest],
	  (var(Index) ->
                call_explicit(check_pairapplied_direct(PList, PListRest, PConsNumber, AppTail, AppRest, Status), ech)

	       ;suspension_to_goal(Index, Cons0, _),
                arg(constraintnum_pos, Cons0, GConsNo),

	        call_explicit(check_pairapplied_directmarked(PList, PListRest, PConsNumber, AppTail, GConsNo, PGoal, RuleNo, Status), ech)
          )
        ),
  ...
  AppTail, _ConsNo, Status) :- !,
    Code = ( nonvar(Applied) ->
	     ...
	     ; % var(Applied)
               ...
               call_explicit(create_applied(Applied, Size),ech),
               Status = notfound
           ), AppTail = [],
...
appropriate_applied(1, double(H1, _H2), Head, RuleNo, [PIndex], Applied, Code, 
  Add_Applied, AppTail, ConsNo, Status) :- !,
    ...
    Code = ( suspension_to_goal(PIndex, PGoal, _),
              arg(constraintnum_pos, PGoal, PConsNumber),
              call_explicit(check_samepairapplied(RuleNo, Applied, Pos-PConsNumber, AppTail, AppRest, Status), ech)
     ),
     Add_Applied = call_explicit(insert_samepairapplied(Pos-PConsNumber, AppTail, AppRest, OtherPos-ConsNo, PGoal, RuleNo), ech).
appropriate_applied(1, _, _, RuleNo, [PIndex], Applied, Code, Add_Applied, AppTail, ConsNo,
   Status) :- !,
     Code = ( suspension_to_goal(PIndex, PGoal, _),
              arg(constraintnum_pos, PGoal, PConsNumber),
              call_explicit(check_pairapplied(RuleNo, Applied, PConsNumber, AppTail, AppRest, Status), ech)
     ),
     Add_Applied = call_explicit(insert_pairapplied(PConsNumber, AppTail, AppRest, ConsNo, PGoal, RuleNo), ech).
...
construct_deletion([DHead|DHs], [Index|Indecies], Constraints, Code) :-
   ...
   Code = (call_explicit(kill_constraint(ConstNo, Index),ech), Code1), 
...
construct_body(a(Head,CHead,MetaArgs,_,AllRest,PIndecies,GConst,Add_Applied,_),
  ExtendArgs,  Nprule, Code, PostFired, TryNext, Module) :-
   ...
   (HeadAction == keep ->
     ...
     ; /* HeadAction \= keep */
       Code = (
         call_explicit(kill_constraint(ConstNo, Index),ech),
         DeleteConstraints,
         BodyGoals
...
% Add_Applied (first arg.) is used to pass extra args for constructing code
	Code = 
	      ...
	      call_explicit(suspend_constraint(Constraint, Head, ConstNo, Prio), ech),
	      call_explicit(insert_samepairapplied(Pos-PConsNumber, AppTail, AppRest, OtherPos-GConst, PGoal, RuleNo), ech)
...
% double headed propagation rule, with different heads
	Code = 
          (var(GConst) ->
	      call_explicit(suspend_constraint(Constraint, Head, ConstNo, Prio), ech),
	      call_explicit(insert_pairapplied(PConsNumber, AppTail, AppRest, GConst, PGoal, RuleNo), ech)
...
construct_writeconstraint_code(SCons, Define, NNameRule) :-
    ...
    Define = (:- erase_macro(F/A), define_macro(F/A, NName/2, [write,goal])).
...
initialise_module_for_chr(Module) :-
   ...
   make_local_array('CHRcstore', global_reference)@Module,

   % get around bug where the reference takes on value from another...
   ...
   local_record('CHRcode')@Module, local_record('CHRconstraints')@Module.


redefine_cdelete_count(Error, Culprit, Module) :-
    (Culprit = make_local_array('CHRcdelete_count'(_N), integer) -> 
...
:- set_event_handler(42,  redefine_cdelete_count/3). 
...
find_partner([SIndex|Rest], SIndex, Constraint, Rest, yes) :- 
    suspension_to_goal(SIndex, Suspended, _),
...
ordered_merge(L0, L1, L) :-
   L0 = [SIndex0|L00],
   L1 = [SIndex1|L11],
   (suspension_to_goal(SIndex0, G0,_) ->
       (suspension_to_goal(SIndex1, G1,_) ->
...
is_in_store([CIndex|L], NewCon) :-
   (suspension_to_goal(CIndex, CGoal,_) ->
...
matching_constraint([CIndex0|_], Constraint, CIndex) :-
	suspension_to_goal(CIndex0, CGoal,_),
